...
class NeuMFProfileMLP(nn.Module):
    ...
    """
    def __init__(self, num_user_features, num_items, latent_dim=32):
...
def main():
    ...
    net = NCFMLP(len(all_users), len(all_contents), latent_dim=32)
    ...
    if not train_user_features_arr:
        ...
        np.random.seed(42)
        torch.manual_seed(42)
        profile_net = NeuMFProfileMLP(num_user_features, num_items, latent_dim=32)
        profile_criterion = nn.MSELoss()
        profile_optimizer = optim.Adam(profile_net.parameters(), lr=0.01, weight_decay=1e-4)

        profile_net.train()
        for epoch in range(20):  # mismas 15 épocas que el NeuMF original
